...
- **Tools**: Executable functions exposed as MCP tools (e.g., m365_list, email_send)
...
@mcp.tool
def m365_list(resource: str, account_id: str | None = None) -> dict[str, Any]:
    """List items of one resource type."""
...
- **Spec first**: the tool surface is defined by `docs/unified-tools/`
  (generated by `scripts/build_unified_tool_specs.py`) and registered from
  its JSON by `tools/registry.py`. Change a tool by editing the builder,
  regenerating and re-testing; never by editing a handler signature.
- **Account isolation**: every Microsoft 365 tool accepts an optional
  `account_id` (ID or email). Omitted means the only signed-in account;
  with several accounts the error lists them. Handlers resolve it once and
  use it for all Graph and cache operations.
- **Breaking changes only in a major version**: renaming or removing a
  tool, parameter or result field, tightening a schema, or changing a
  default is a breaking change. It ships only in a major release and is
  listed in `CHANGELOG.md`. Additive, optional changes may ship in minor
  releases.
- **Typed results**: every tool declares an `outputSchema` and returns
  `structuredContent` that validates against it, plus one text block equal
  to its `summary`. Results are projections: no raw Graph objects, no
  `@odata.*` fields, no cache or other internal fields.
- **Confirm gates**: tools that send mail, notify other people, share data
  or destroy data take `confirm` (default `false`) and refuse unless it is
  `true`, exactly as the spec's `meta.confirm` / `confirm_rule` state.
  Descriptions tell the model to ask the user first. `confirm` is not a
  substitute for host-side approval of `dangerous`/`critical` tools.
