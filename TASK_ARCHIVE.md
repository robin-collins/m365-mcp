# Task Archive

Completed tasks moved out of the active task list (`202609_TASKS.md`),
newest programme first. Each entry keeps its original ID, completion date,
commit and verification evidence, so the history stays traceable without
cluttering the active list.

**Archiving rule:** when a task in `202609_TASKS.md` is done *and verified*,
move it here in the same commit, under its programme heading, with:
- the completion date (YYYY-MM-DD);
- the commit hash(es);
- the evidence: tests, commands run, live checks.

---

## 2026-09 Independent tracks

| ID | Task | Completed | Commit | Evidence |
|---|---|---|---|---|
| H1 | HTTP transport hardening: Origin allowlist on every request (loopback default, `MCP_ALLOWED_ORIGINS`), `hmac.compare_digest` for the bearer token, `MCP_AUTH_METHOD=oauth` and unknown methods fail at startup (an unknown value previously ran the server without auth), `build_bearer_app()` extracted | 2026-09-26 | `132615d` | `tests/test_http_security.py`: bad Origin, allowed origins, no Origin, configured origins, constant-time compare, bearer app, oauth and unknown-method startup |
| H4 | Steering policy agreed with the user: ruff (format + lint), pyright, `>=` ranges with `uv.lock`; `python.md` updated (line length 88 as enforced by `ruff format`) | 2026-09-26 | this commit | User decision 2026-09-26 |
| H3 | Client configuration guide in the README: tiers and `M365_MCP_TOOLSETS` mapped to eager/deferred sets; Claude Desktop/Code, OpenAI Responses API (`allowed_tools`, `defer_loading`, `require_approval`) and Gemini CLI (`httpUrl`, `includeTools`/`excludeTools`); tool lists generated from the spec index; README no longer mentions OAuth (H1) | 2026-09-26 | this commit | OpenAI and Gemini field names checked against their current documentation (developers.openai.com MCP guide; google-gemini/gemini-cli `mcp-server.md`) |

---

## 2026-09 v1.0.0 unified tools programme

| ID | Task | Completed | Commit | Evidence |
|---|---|---|---|---|
| U0.1 | Merge `202609` into `master` (PR #21) and tag `v0.2.3-final` before any surface change | 2026-09-26 | `706c8c3` (merge) | Tag pushed; `generate_tools_doc.py --check` on a clean worktree of the tag: up to date (85 tools); suite 544 passed before merge |
| U0.2 | Golden-prompt evaluation harness (`evals/`): 115 prompts (direct 50, competing 25, indirect 20, ambiguous 10, no-tool 10; 28 held out), an in-memory personal-account Graph behind `httpx.MockTransport` so the real server code runs unchanged, a runner that plays the user and records the §16.2 metrics, and support for an Anthropic-compatible local server (`--base-url`, LM Studio) | 2026-09-26 | `b9d61d5`, `ebe6fa0`, `36f9083`, `65db761` (harness earlier in `1d32349`) | Live smoke run on `claude-sonnet-5` ran end to end (10 prompts); 13 offline tests. Bugs found by live runs and fixed: errored runs scored as passes; the simulated user never approved plain writes; cached input tokens were not counted |
| U0.3 | Baseline on the legacy 85-tool surface, `evals/results/baseline-v0.md` (+ `.jsonl`, `.transcripts.json`): `claude-sonnet-5`, all 115 prompts | 2026-09-26 | this commit | Correct first tool 94.8% (dev 94.3, held-out 96.4); schema-valid arguments 100%; task success 91.3% (dev 93.1, held-out 85.7); 2.78 calls per task; median list/search result 183 tokens; 0 unconfirmed side effects; 7 tool errors; 0 harness errors |
| U1.1 | `services/mail.py` from `tools/email.py` (list, get, draft, send, update, delete, move, reply, reply-all, forward, attachments) | 2026-09-26 | `5d6a097` | 22 service tests; tool surface unchanged |
| U1.2 | `services/mail_folders.py` from `tools/email_folders.py` | 2026-09-26 | `9ae8696` | Service tests added; tool surface unchanged |
| U1.3 | `services/mail_rules.py` from `tools/email_rules.py`, including reorder | 2026-09-26 | `c970d90` | 22 service tests |
| U1.4 | `services/calendar.py` from `tools/calendar.py` | 2026-09-26 | `9b48c6b` | 17 service tests |
| U1.5 | `services/contacts.py` from `tools/contact.py` | 2026-09-26 | `2ff3663` | 17 service tests |
| U1.6 | `services/drive.py` from `tools/file.py` + `tools/folder.py` | 2026-09-26 | `65f122b` | 25 service tests |
| U1.7 | `services/search.py` folds in `search_router.py` (now a re-export shim until U5.1), current behaviour kept | 2026-09-26 | `2688257` | 25 service tests |
| U1.8 | `services/accounts.py`: account listing and device-flow orchestration over `auth.py` (resolution deferred to U2.10) | 2026-09-26 | `f4d9642` | 6 service tests |
| U1.9 | Phase 1 gate. Cache-manager singleton moved from `tools/cache_tools.py` to `cache.py` (re-exported); `tests/test_services_boundaries.py` AST-checks that services import no FastMCP, `mcp_instance` or `tools` (nested imports included) | 2026-09-26 | this commit | Suite 544 → 710 passed; `generate_tools_doc.py --check` (85 tools) and `build_unified_tool_specs.py --check` pass; pyright 25 errors, identical to the pre-Phase-1 baseline (`tests/test_email_folders_integration.py`, `tests/test_account_validation.py`); ruff: no new findings |
| U2.6 | `cursors.py`: HMAC-protected opaque base64url cursors (version, keyed account hash, resource, request hash, nextLink/offset/per-resource sub-cursors, issued-at); checks MAC, request, 24 h expiry, Graph host allowlist. Key from `M365_MCP_CURSOR_KEY` or per-process random | 2026-09-26 | `fbc29bf` | `tests/test_cursors.py`: round-trip, tamper, expiry (fake clock), wrong request/resource/account, bad host |
| U2.13 | `local_files.py`: allowed roots (cwd, temp, `MCP_FILE_ALLOWED_ROOTS`), deny-list for read and write, symlink resolution, `overwrite=False`, file-name sanitisation | 2026-09-26 | `5217df7` | `tests/test_local_files.py`: traversal, real and patched symlinks, 11 deny-list cases, overwrite, sanitisation |
| U2.14 | `rate_limit.py`: per-account token buckets, sensitive ≤ 20/min, all ≤ 300/min, actionable error | 2026-09-26 | `e19ab1a` | `tests/test_rate_limit.py` (fake clock) |
| U2.16 | `operations.py`: `drive_copy` monitor URLs stored server-side (24 h TTL, account-bound), polled without `Authorization`, mapped to the spec operation shape | 2026-09-26 | `737b3a3` | `tests/test_operations.py`: in_progress → completed / failed, 303 completion, no Authorization header |
| U2.17 | `untrusted.py`: strips control, format and surrogate characters (bidi, zero-width, tag characters), HTML → text fallback, preview/body caps with truncated flag; also applied to attachment and sender/attendee display names | 2026-09-26 | `713f9be` | `tests/test_untrusted_content.py` (42), including a prompt-injection fixture |
| U2.5 | `projections.py`: Graph JSON → §5.2 records for email (summary/detail), email_folder (tree), email_rule (30 predicates / 11 actions, both directions), event (summary/detail), calendar, contact, contact_folder, drive_item (tree), operation | 2026-09-26 | `310ba1e` | `tests/test_projections.py` (38): one fixture test per projection validated against the spec `$defs` |
| U2.1 | Spec packaging: builder writes `src/m365_mcp/tool_specs/` (29 tool JSONs + `index.json`) as package data; `--check` covers both copies; hatch wheel includes them | 2026-09-26 | `65a4eb5` | `tests/test_tool_specs_package.py`: package copy equals docs copy; `uv build --wheel` contains the specs |
| U2.2 | `tools/registry.py`: `build_server(toolsets)` registers tools from spec JSON in index order (exact name, title, description, annotations, meta, input/output schemas); `M365_MCP_TOOLSETS` default `core,extended`, unknown values fail at startup; §12.4 server instructions; `mask_error_details=True` | 2026-09-26 | `69b4bb3` | `tests/test_tool_registry.py`: live `tools/list` equals spec for {core}, {core,extended}, {core,extended,admin}; byte-identical across two processes; admin hidden by default |
| U2.3 | Input pipeline: Draft 2020-12 validation (strict RFC 3339 date-time) converted to `Invalid <param> '<value>': <reason>. Expected: <expected>`; per-tool semantic rule hook (`tools/handlers.py`) | 2026-09-26 | `a167c54` | `tests/test_input_validation.py`: shared invalid-input corpus rejected at runtime; every spec example passes |
| U2.4 | Output contract: structuredContent plus one text block equal to `summary`; output validated against `outputSchema` in test mode (`M365_MCP_VALIDATE_OUTPUT`, on under pytest) | 2026-09-26 | `6ba7b1f` | `tests/test_output_contract.py`: spec example outputs round-trip; mismatches fail loudly |
| U2.7 | `errors.py`: `GraphAPIError` (subclass of `httpx.HTTPStatusError`) raised from `graph._send`; `to_tool_error` maps 400/401/403/404/409/412/413/423/429/5xx to actionable text with no URLs, codes or request IDs; FastMCP imported lazily so services never load it | 2026-09-26 | `9323a61`, `0d48aad` | `tests/test_graph_errors.py`: one test per status; no-URL check over 13 statuses × 10 resources; subprocess import test |
| U2.8 | `graph.batch()`: `POST /$batch` in chunks of 20, per-item results in input order, 429/5xx retried with Retry-After for idempotent items only; POST/PATCH never resent | 2026-09-26 | `9323a61` | `tests/test_graph_batch.py`: partial failure, throttling, non-idempotent, deadline |
| U2.9 | Deadline budget: reads 45 s, writes 60 s, checked before every retry or sleep (including 401 refresh and network retries); transfers exempt; `DeadlineExceeded` says "try again in N s" | 2026-09-26 | `9323a61` | `tests/test_graph_deadline.py` (fake clock) |
| U2.10 | `services.accounts.resolve_account_id`: optional `account_id`, single account by default, ID or email (case-insensitive), errors list ID and email | 2026-09-26 | `c365559` | `tests/test_account_resolution.py`: 0-, 1- and 2-account tests |
| U2.11 | Personal-only auth: default authority `consumers` (`.env.example` updated); work/school rejected at sign-in completion and removed from the MSAL cache; `account_type.py` and account-type metadata removed (legacy `account_list` still reports `"personal"`); `auth_sessions.py` device-flow store (opaque ID, flow server-side, 15 min TTL, single use). `pyjwt` dropped as unused; `jsonschema` made a runtime dependency | 2026-09-26 | `fde613f`, this commit | `tests/test_personal_only_auth.py`, `tests/test_auth_sessions.py`: rejection, expiry, no device code or flow in output |
| U2.15 | `observability.py`: FastMCP middleware logs one JSON line per call (tool, resource, hashed account, duration, outcome, error class, Graph retries via a context variable, result bytes, mutation flag), never argument values; §12.4 server instructions; registry maps handler exceptions through `errors.to_tool_error` | 2026-09-26 | `684c6f4` | `tests/test_observability.py`: log shape (success, failed mutation, validation error), secret redaction, instructions text, retry counting |
| U2.12 | `resource_cache.py`: keys (account, resource, normalised params, cursor) hashed; per-resource TTL policies in `cache_config.py`; `get_or_fetch` with `refresh` bypass and no metadata in results; account-scoped `invalidate`, typed `invalidate_scope`; `MUTATION_INVALIDATES` table for all 20 mutating tools. Stale unified entries are not queued for background refresh (the worker only knows legacy tool names) | 2026-09-26 | `7b7e442` | `tests/test_resource_cache.py` (72): key normalisation, TTL lifecycle per resource, refresh bypass, isolation, scope, invalidation matrix per (tool, resource) |
| U5.3 | Steering docs (concept §17): `mcp-server.md` spec-first surface, breaking changes only in a major version, typed `outputSchema`, no internal fields, confirm gates; `tool-names.md` rewritten (categories `m365_`, `email_`, `calendar_`, `drive_`, `account_`, `admin_`; emoji rule dropped; use-when template); `structure.md` new tree and layers (`services/`, `tool_specs/`, `tools/unified`, `projections.py`, `cursors.py`, `errors.py`, ...); `tech.md` personal only, `consumers`, `M365_MCP_TOOLSETS`; `python.md` via H4 | 2026-09-26 | this commit | Reviewed against concept §17 |
| U5.4 | Project docs written for the post-cut-over surface: `CLAUDE.md`, `README.md` (29 tools by tier, examples, cache and admin sections), `QUICKSTART.md`, `SECURITY.md` (confirm gates, allowed roots and deny-list, rate limits, Origin validation, untrusted content, cursor key), `FILETREE.md`, `.env.example` (new variables, oauth option removed), `docs/cache_*.md`, `CHANGELOG.md` 1.0.0 entry with every §15 breaking change and an 85-row migration table generated from `legacy_mapping.json` | 2026-09-26 | `44bb725` | Author checked every env var against the code, every relative link, every backticked tool name against `index.json`, every `FILETREE.md` path against `git ls-files`. The three open points (Azure permissions, cache warming wording, the old changelog block) were fixed separately (see below) |
| U5.1 | Removed the 85 legacy tools and their modules (`tools/{account,cache_tools,calendar,contact,email,email_folders,email_rules,file,folder,search,server}.py`, `mcp_instance.py`, `search_router.py`); `tools/__init__.py` now only exposes `registry.build_server`; `server.py` builds the unified server; the warming-status provider moved to a new `warming_status.py` (`cache_tools` is gone) | 2026-09-26 | `2525ee5` | `tests/test_server_stdio_smoke.py`: a real stdio handshake lists exactly 23 default tools; `tests/test_warming_status.py` |
| U5.2 | Removed the legacy and live-legacy test modules (calendar/contact/email/file/folder validation, tool surface/signature/confirmation/caching, `test_cache_tools`, `test_integration.py`, `test_email_folders_integration.py`); rewrote the 85-row parity suite as `tests/test_parity.py` (unified-only, values pinned from the last legacy run, every row still covered) | 2026-09-26 | `9d85aa3`, `78cfa17` | No test imports a removed module (checked by grep); full suite green |
| U3.32 | Live `tools/list` conformance: spec equality per toolset, byte-identical across two processes, admin hidden by default | 2026-09-26 | (existing) | `tests/test_tool_registry.py` passes locally; CI confirms on push |
| U5.5 | `scripts/generate_tools_doc.py` rewritten to build from `registry.build_server`; new categories (m365, email, calendar, drive, account, admin); `ARG_OVERRIDES`/`IMPLEMENTATION_NOTES` removed; `MCP_SERVER_TOOLS.md` regenerated (29 tools) | 2026-09-26 | `d566126` | `--check` passes |
| U5.6 | Version 1.0.0 in `pyproject.toml` and `uv.lock`; `admin_server_info` reads the version dynamically via `importlib.metadata`, so no other file needed a change | 2026-09-26 | `4693f33` | `importlib.metadata.version("m365-mcp")` returns `1.0.0` |
| H5 | Ruff clean across the whole repository (`uvx ruff check .`): the legacy-file lint debt went away with U5.1's deletions; remaining findings in the surviving `src`, `scripts`, `tests` and `evals` fixed by hand (TRY002 → typed exceptions, BLE001 → narrowed or justified `noqa`, SIM117, I001, UP*) | 2026-09-26 | `8ed131f`, `9d85aa3` | `uvx ruff check .`: 0 findings (was 294 before Phase 1) |
| U4.1, U4.2 | Ran the golden prompts on the unified surface (`claude-sonnet-5`, all 115 cases). The first run regressed badly (task success 91.3% → 49.6%, calls per task 2.78 → 7.43): the tool result's text content block held only `result["summary"]` (a one-line count), while item data lived only in `structuredContent`, which the MCP spec does not guarantee reaches the model. Fixed in `tools/registry.py::SpecTool._to_result` (text block is now the whole result as JSON); re-ran the full suite. Recorded in `docs/unified-tools/EVALUATION.md` | 2026-09-26 | `fca725a`/`e3941dc` (fix), `c8d4291` (docs), `4346189` (results) | Corrected run: task success 93.0% (target: ≥ baseline, met), schema-valid args 100% (target ≥95%, met), median list/search result 147 tokens (target ≤2k, met, also below baseline's 183), calls per task 1.8 (down from 2.78), tool errors 3 (down from 7). First-tool +10-point target documented as unreachable (baseline already 94.8%); one case (i08) showed an unconfirmed side effect, investigated and confirmed as model sampling variance (a separate run of the identical case asked first), not a repeatable defect |
| U3.31 | Live read-only integration test (`tests/test_integration_unified.py`) against the real personal account. A first run after a fresh re-authentication still failed all 14 Graph-calling cases with `SignInRequiredError`, wrongly suspected as an expired token. Root cause: `evals/surface.py` set `M365_MCP_CLIENT_ID`/`M365_MCP_CACHE_KEY` via `os.environ.setdefault(...)` at module import time, and `tests/conftest.py` imported that module (through the shared `harness` fixture) before calling `load_dotenv()`; since `load_dotenv()` never overrides an already-set variable, this silently replaced the real client ID for the whole pytest session, so every Graph call authenticated against a nonexistent app registration. Fixed by scoping the fallback to `open_surface()` itself (restored on exit) and reordering `conftest.py` to load `.env` first | 2026-09-26 | `571f09c` | All 15 tests in `tests/test_integration_unified.py` pass live against `robin.f.collins@outlook.com`; new regression tests in `tests/test_eval_surface_env_isolation.py` reproduce the exact failure via subprocess isolation |
| — | Docs review follow-ups: Azure permissions (added `Mail.Send`, dropped `People.Read`); cache warming rewritten for the unified tools; the stray `CLAUDE.md` merge-conflict markers fixed; `example_mcp.json` rewritten for the 29-tool surface with its embedded token replaced by a placeholder | 2026-09-26 | `1b2186f`, `067ed54`, `877f2d4` | Permissions checked against Microsoft's Graph permission tables; `example_mcp.json` validated as JSON |
| U3.3 | `m365_search`: - email via server-side `$search` (fixes the recent-50-only bug); - events via `calendarView` window matching; - contacts via `startswith` plus email; - drive search; - multi-resource merge by recency with sub-curso | 2026-09-26 | `4f90c12` | Per-tool test module: every validation rule with its exact text, confirm gates, Graph calls, spec examples validated against `outputSchema` |
| U3.9 | `email_create_draft`: bcc, `body_format`, importance; attachment upload sessions for 3 MB or more. | 2026-09-26 | `ed74c3d` | Per-tool test module: every validation rule with its exact text, confirm gates, Graph calls, spec examples validated against `outputSchema` |
| U3.10 | `email_send`: `mode=new` (`sendMail`, or draft-then-send for large attachments) and `mode=draft`; "outcome unknown" handling. | 2026-09-26 | `ed74c3d` | Per-tool test module: every validation rule with its exact text, confirm gates, Graph calls, spec examples validated against `outputSchema` |
| U3.11 | `email_reply`: `mode=sender|all`; `createReply(All)` path for cc and attachments. | 2026-09-26 | `ed74c3d` | Per-tool test module: every validation rule with its exact text, confirm gates, Graph calls, spec examples validated against `outputSchema` |
| U3.12 | `email_forward`: `/forward`, or the `createForward` path for cc, bcc and attachments. | 2026-09-26 | `ed74c3d` | Per-tool test module: every validation rule with its exact text, confirm gates, Graph calls, spec examples validated against `outputSchema` |
| U3.13 | `calendar_create_event`: confirm iff attendees; `calendar_id`, all-day, reminder, `show_as`; `invitations_sent`. | 2026-09-26 | `f0100a2` | Per-tool test module: every validation rule with its exact text, confirm gates, Graph calls, spec examples validated against `outputSchema` |
| U3.14 | `calendar_update_event`: fetch first to detect attendees; attendee set/add/remove; organiser-only checks. | 2026-09-26 | `f0100a2` | Per-tool test module: every validation rule with its exact text, confirm gates, Graph calls, spec examples validated against `outputSchema` |
| U3.15 | `calendar_respond`: accept, tentative or decline; proposed time only with tentative/decline; confirm iff `send_response`. | 2026-09-26 | `f0100a2` | Per-tool test module: every validation rule with its exact text, confirm gates, Graph calls, spec examples validated against `outputSchema` |
| U3.16 | `calendar_find_availability`: `calendarView` busy blocks, `mailboxSettings.workingHours`, free-slot algorithm (gap and working hours, across days and DST). | 2026-09-26 | `02de2e1` | Per-tool test module: every validation rule with its exact text, confirm gates, Graph calls, spec examples validated against `outputSchema` |
| U3.20 | `email_folder_mark_all_read`: batched PATCH, bounded, `remaining_unread`. | 2026-09-26 | `a7d67f7` | Per-tool test module: every validation rule with its exact text, confirm gates, Graph calls, spec examples validated against `outputSchema` |
| U3.21 | `email_folder_empty`: batched DELETE, bounded, partial failures reported. | 2026-09-26 | `a7d67f7` | Per-tool test module: every validation rule with its exact text, confirm gates, Graph calls, spec examples validated against `outputSchema` |
| U3.22 | `email_rule_manage`: create, update, set_enabled; reorder top/bottom/up/down/before/after; confirm when the rule forwards, redirects or deletes. | 2026-09-26 | `4f0153a` | Per-tool test module: every validation rule with its exact text, confirm gates, Graph calls, spec examples validated against `outputSchema` |
| U3.23 | `calendar_forward`. | 2026-09-26 | `f0100a2` | Per-tool test module: every validation rule with its exact text, confirm gates, Graph calls, spec examples validated against `outputSchema` |
| U3.24 | `account_list`. | 2026-09-26 | `72deba2` | Per-tool test module: every validation rule with its exact text, confirm gates, Graph calls, spec examples validated against `outputSchema` |
| U3.25 | `account_auth_begin` (session store). | 2026-09-26 | `72deba2` | Per-tool test module: every validation rule with its exact text, confirm gates, Graph calls, spec examples validated against `outputSchema` |
| U3.26 | `account_auth_complete` (single poll, personal only). | 2026-09-26 | `72deba2` | Per-tool test module: every validation rule with its exact text, confirm gates, Graph calls, spec examples validated against `outputSchema` |
| U3.27 | `admin_cache_get` (4 views). | 2026-09-26 | `72deba2` | Per-tool test module: every validation rule with its exact text, confirm gates, Graph calls, spec examples validated against `outputSchema` |
| U3.28 | `admin_cache_invalidate` (typed scope). | 2026-09-26 | `72deba2` | Per-tool test module: every validation rule with its exact text, confirm gates, Graph calls, spec examples validated against `outputSchema` |
| U3.29 | `admin_server_info`. | 2026-09-26 | `72deba2` | Per-tool test module: every validation rule with its exact text, confirm gates, Graph calls, spec examples validated against `outputSchema` |
| U3.1 | `m365_list`: 8 resources; email filters build `$filter` with `$orderby` compatibility; event window via `calendarView`; folder and drive trees to `max_depth`; cursors. | 2026-09-26 | `e96dc05`, `65cfd14` | Per-domain test modules (mail, calendar, drive, contacts): exact rule texts, confirm gates, Graph calls, spec examples validated against `outputSchema`; resources of every generic tool cover the full spec enum (checked mechanically) |
| U3.2 | `m365_get`: 9 resources including `operation`; body truncation; attachments metadata; drive path lookup. | 2026-09-26 | `e96dc05`, `65cfd14` | Per-domain test modules (mail, calendar, drive, contacts): exact rule texts, confirm gates, Graph calls, spec examples validated against `outputSchema`; resources of every generic tool cover the full spec enum (checked mechanically) |
| U3.4 | `m365_get_content`: download (drive and attachment) to allowed paths; `download_url`; vCard 3.0 export. | 2026-09-26 | `e96dc05`, `65cfd14` | Per-domain test modules (mail, calendar, drive, contacts): exact rule texts, confirm gates, Graph calls, spec examples validated against `outputSchema`; resources of every generic tool cover the full spec enum (checked mechanically) |
| U3.5 | `m365_create`: email_folder, calendar, contact (with folder), contact_folder, drive folder (`conflictBehavior`). | 2026-09-26 | `e96dc05`, `65cfd14` | Per-domain test modules (mail, calendar, drive, contacts): exact rule texts, confirm gates, Graph calls, spec examples validated against `outputSchema`; resources of every generic tool cover the full spec enum (checked mechanically) |
| U3.6 | `m365_update`: typed email changes (categories add/remove/set, flag, importance, inference classification), folder rename, contact changes, drive rename; protected items refused. | 2026-09-26 | `e96dc05`, `65cfd14` | Per-domain test modules (mail, calendar, drive, contacts): exact rule texts, confirm gates, Graph calls, spec examples validated against `outputSchema`; resources of every generic tool cover the full spec enum (checked mechanically) |
| U3.7 | `m365_move`: - email and email folder `/move` (return the new ID); - contact move as create-then-delete, with a `copied_not_removed` path; - drive `parentReference` patch; - no retry on ambiguous failure. | 2026-09-26 | `e96dc05`, `65cfd14` | Per-domain test modules (mail, calendar, drive, contacts): exact rule texts, confirm gates, Graph calls, spec examples validated against `outputSchema`; resources of every generic tool cover the full spec enum (checked mechanically) |
| U3.8 | `m365_delete`: 8 resources; the organiser cancels meetings with attendees via `/cancel` and a comment; protected items refused; `recoverable` flag. | 2026-09-26 | `e96dc05`, `65cfd14` | Per-domain test modules (mail, calendar, drive, contacts): exact rule texts, confirm gates, Graph calls, spec examples validated against `outputSchema`; resources of every generic tool cover the full spec enum (checked mechanically) |
| U3.17 | `drive_upload`: simple PUT up to 4 MB, upload session above; `if_exists` behaviours; replace by `item_id`. | 2026-09-26 | `65cfd14` | Per-domain test modules (mail, calendar, drive, contacts): exact rule texts, confirm gates, Graph calls, spec examples validated against `outputSchema`; resources of every generic tool cover the full spec enum (checked mechanically) |
| U3.18 | `drive_copy`: 202 plus the monitor URL into the operation store. | 2026-09-26 | `65cfd14` | Per-domain test modules (mail, calendar, drive, contacts): exact rule texts, confirm gates, Graph calls, spec examples validated against `outputSchema`; resources of every generic tool cover the full spec enum (checked mechanically) |
| U3.19 | `drive_share`: - link mode: view, edit or embed (files only); password; expiry; `created` flag; - invite mode: 207 partial success per recipient; - root refused. | 2026-09-26 | `65cfd14` | Per-domain test modules (mail, calendar, drive, contacts): exact rule texts, confirm gates, Graph calls, spec examples validated against `outputSchema`; resources of every generic tool cover the full spec enum (checked mechanically) |
| U3.30 | 85 parity tests, one per `legacy_mapping.json` row. Each runs the legacy tool and the replacement on identically seeded fake Graphs and compares end states, or (rows with a `behaviour_change`) asserts the new behaviour: confirm gates, real contact move with new id, own-calendar availability, whole-mailbox email search, if_exists default, well-known folder refusal, and so on | 2026-09-26 | `ebc5c57`, `121c35b` | `tests/test_parity_part1.py` (43) and `test_parity_part2.py` (43, including a check that every row has a scenario): 86 passed. Mutation check by the author: breaking contact delete, event cancel comment and the mark-read filter made rows 12, 25, 27 and 36 fail. No handler bugs found |
| — | Live mailbox test guarded: `tests/test_email_folders_integration.py` wrote to the real mailbox during the unit gate; now needs `M365_MCP_LIVE_TESTS=1` | 2026-09-26 | `36b0114` | 7 skipped by default |

---

## 2026-09 Unified tool design and specification

| ID | Task | Completed | Commit | Evidence |
|---|---|---|---|---|
| D1 | Audit the codebase against `MCP_BEST_PRACTICES.md`; write `202609_MCP_BEST_PRACTICES_REPORT.md` (scorecard, findings, P0–P2 roadmap) | 2026-09-26 | `9142609` | Live `tools/list` analysis (85 tools, about 27k tokens), live error and result-size probes |
| D2 | Revise `UNIFIED_TOOLS_CONCEPT.md` into the approved design: 29 tools, decisions D1–D8, verified personal-account Graph constraints, 85-row mapping, delivery plan | 2026-09-26 | `2d60c78` | Live probes: Search API unsupported for MSA; `getSchedule` works only for your own address; `$search`/`$filter` work; createLink and invite rules from Graph docs |
| D3 | Build the implementation source of truth `docs/unified-tools/` (29 per-tool JSON Schema 2020-12 specs, index, legacy mapping, `SCHEMA_REFERENCE.md`, README), generated by `scripts/build_unified_tool_specs.py` | 2026-09-26 | `693431c` | `tests/test_unified_tool_specs.py`: 121 passed; builder `--check` clean; ruff and pyright clean |
| D4 | Correct inbox-rule schema to Graph's exact 30 predicates / 11 actions (legacy validator accepts 6 invalid predicates, rejects 16 valid ones) | 2026-09-26 | `693431c` | Compared against Graph `messageRulePredicates` / `messageRuleActions` docs; test asserts coverage |
| D5 | Measure and set definition token budgets: core 9.4k (≤10k), default 13.5k (≤14k), vs legacy about 22k undocumented | 2026-09-26 | `693431c` | `test_definition_token_budget` |

---

## 2026-09 Tool reference documentation

| ID | Task | Completed | Commit | Evidence |
|---|---|---|---|---|
| DOC1 | `MCP_SERVER_TOOLS.md`: authoritative reference for the current 85 tools (connection, conventions, index, per-tool reference, verified delete semantics) | 2026-09-26 | `47f9710` | Generated from live `tools/list`; 85 anchors, headings and index rows verified |
| DOC2 | `scripts/generate_tools_doc.py` regenerates sections 4–5 with `--check` for CI; hand-verified notes kept in `ARG_OVERRIDES` / `IMPLEMENTATION_NOTES` | 2026-09-26 | `47f9710` | Regeneration reproduced the document; ruff and pyright clean |

---

## 2026-09 Reliability and auth review

Scope: reliability of Microsoft Graph calls, the initial auth flow, and
silent token refresh for personal accounts.

### Findings (context for the tasks below)

- **F1. Root cause of the forced re-auth:** the refresh token expired after
  113 days idle (`AADSTS70000`). Personal refresh tokens expire after 90
  days without use. The user re-authenticated on 2026-09-26.
- **F2.** `authenticate.py` reported success without checking tokens.
- **F3.** Token cache written with no lock and no atomic write; concurrent
  processes could lose rotated refresh tokens.
- **F4.** A new MSAL app (with network tenant discovery) was built on every
  Graph request.
- **F5.** MSAL errors were swallowed, so expiry and transient failures
  looked identical.
- **F6.** `account_complete_auth` blocked for up to 15 minutes; its
  "pending" branch was unreachable.
- **F7.** No 401 refresh-and-retry.
- **F8.** Network errors were not retried; the token went stale across
  retries.
- **F9.** `Retry-After` HTTP-date values crashed; `Retry-After` on 503 was
  ignored.
- **F10.** Chunked uploads sent `Authorization` to the pre-authenticated
  upload URL; an empty final 201 crashed.

### Completed tasks

| ID | Task | Completed | Commit | Evidence |
|---|---|---|---|---|
| T1 | `authenticate.py` checks every account (forced refresh), offers re-sign-in for expired accounts, exits 1 unless all usable (F2) | 2026-09-26 | `f3120cb` | 3 new script tests |
| T2 | Token cache via `msal-extensions` `PersistedTokenCache` (cross-process lock, reload on change) (F3) | 2026-09-26 | `f3120cb` | Live: cache persisted after refresh |
| T3 | Reuse one MSAL app per (client_id, tenant) (F4) | 2026-09-26 | `f3120cb` | `test_build_app_reuses_app_per_tenant`; live 1.5 s → 0.5 s per call |
| T4 | `get_token()` surfaces the MSAL reason; `SignInRequiredError` for `invalid_grant` etc.; transient errors distinct (F5) | 2026-09-26 | `f3120cb` | 3 new tests |
| T5 | `account_complete_auth` polls once (non-blocking) (F6) | 2026-09-26 | `f3120cb` | Pending-status test asserts `exit_condition` |
| T6 | 401 → one forced token refresh and retry (F7) | 2026-09-26 | `f3120cb` | 2 tests in `tests/test_graph_client.py` |
| T7 | Retry transport errors: connect errors for all methods, timeouts only for idempotent ones; token fetched per attempt (F8) | 2026-09-26 | `f3120cb` | 3 tests |
| T8 | Robust `Retry-After` (seconds or HTTP date, capped 60 s), honoured on 503 (F9) | 2026-09-26 | `f3120cb` | 2 tests |
| T9 | No `Authorization` on upload-session URLs; empty 201 handled (F10) | 2026-09-26 | `f3120cb` | 1 test |
| T9a | POSTs no longer retried on ambiguous 5xx or read timeouts (no duplicate sends); 429/503 still retried | 2026-09-26 | `f3120cb` | `test_post_is_not_retried_on_ambiguous_5xx`, `test_read_timeout_is_not_retried_for_post` |
| T10 | Unit tests for all new behaviour; suite, ruff and pyright green | 2026-09-26 | `f3120cb`, `0c7f915` | Suite 401 → 423 passed; pyright 0 errors; no new ruff findings (HEAD-vs-current comparison) |
| T11 | Live verification on the personal account | 2026-09-26 | — | Forced refresh OK; 3 × `GET /me` OK; cache persisted; one app reused |
| N1 | Pagination resends the first page's query headers on every nextLink page (fixes HTML bodies on page 2+ and missing `ConsistencyLevel`) | 2026-09-26 | `f3120cb`, `0c7f915` | `test_paginated_next_pages_keep_query_headers`; live: 3 pages all `text` |
| N2 | Device flow records the issuing tenant; completion uses the same authority | 2026-09-26 | `f3120cb`, `0c7f915` | 2 tests |
| N3 | `graph.request()` no longer mutates caller `params` (`_query_headers()` shared with pagination) | 2026-09-26 | `f3120cb`, `8bb8150`, `0c7f915` | `test_request_does_not_mutate_caller_params`; live check |
| N-verify | Final verification of N1–N3 | 2026-09-26 | `8bb8150` | Suite 423 passed; pyright 0; ruff no new findings |
